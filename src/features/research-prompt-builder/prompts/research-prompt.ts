...
export function buildResearchPromptCompilerPrompt(input: {
  ...
  const instructions = [
    ...
    "",
    "Density: Be concise and information-dense. Use additional length only where company-specific research controls, evidence requirements, or report instructions require it. Do not repeat methodology.",
    "",
    ...
    "- The opportunity comes first. Measurement validates the opportunity; it does not define it. Per experiment: one commercial bridge after value (or \"none warranted\") — do not require a second duplicative per-experiment CTA field. Report-level: one primary platform and one strategic CTA hypothesis derived from the research (including none/weak when unsupported).",
    "- Stop line in qualityCheckBeforeSubmission: return the completed research output only; do not propose additional workflows. (The formatter also appends a hardened anti-meta footer — still include the stop line in §8.)",
    "",
